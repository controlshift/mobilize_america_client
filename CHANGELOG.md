...
### Added
- `RateLimitedError` now has an `retry_after` attribute, with the value from the Retry-After header if available.

## [1.1.0] - 2026-03-05

### Added
- Responses with 429 status code now produce a more specific `RateLimitedError` exception (inherits from `ClientError`)
